...
class Device extends EventEmitter {
    ...
    onTunnelOnline = () => {
        ...
        if (this.offlineChecker) {
            this.offlineChecker.cancel();
            this.offlineChecker = undefined;
        }
    }

    onTunnelOffline = () => {
        log.info(this.model, '设备下线');
        ...
        if (!this.offlineChecker)
            //validate会卡线程
            this.offlineChecker = interval.check(5001, ()=>{
                //TODO 应该只检查离线
                this.validate();
            });
    }
    ...
    addCollector(c) {
        const col = collector.create(this.adapter, c);
    ...
    validate() {
        this.validators.forEach(v => {
            try {
                ...
                this.error = err.message;
                log.error({error: err.message, validator: v}, '校验执行错误')
                return;
            }
            //以下是不合法处理逻辑
