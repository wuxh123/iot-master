...
class Device extends EventEmitter {
    ...
    addVariable(v) {
        //已经有变量的就不再重复加了
        if (this.variables.hasOwnProperty(v.name))
            return;
        const that = this;
    ...
    initValidator() {
        //先标记历史记录要删除
        this.validators.forEach(v => v._removable = true);
        //如果有重复的，则不添加
        this.element.validators.forEach(v => {
            let has = false;
            const val = this.validators.find(vv => _.isEqual(vv.model, v))
            if (val) {
                //有同样的，则不需要删除
                delete val._removable;
            } else {
        ...
        for (let i = this.validators.length - 1; i >= 0; i--) {
            if (this.validators[i]._removable)
