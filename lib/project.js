    ...
    addUserJob(j) {
        ...
        job.on('execute', (command, params)=>{
            this.execute(command, params).then(() => {
                createEvent({project_id: this.model._id, event: '定时任务：' + command})
            }).catch(err => {
                createEvent({project_id: this.model._id, event: '定时任务：' + command + ' 失败：' + err.message})
            });
        })
    }
