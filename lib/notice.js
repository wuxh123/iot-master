...
exports.notice = async function (alarm) {
    ...
    pipeline.push({$unwind: {path: '$user'}}) //, preserveNullAndEmptyArrays: true} 找不到用户 就过滤掉 或者 删除无效订阅
...
async function sendVoice(alarm, cellphone) {
    log.info({cellphones: alarm.cellphone, cellphone}, 'send voice');
