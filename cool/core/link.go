...
import (
	...
	"git.zgwit.com/zgwit/iot-admin/cool"
...
func newLink(ch cool.Tunnel, conn net.Conn) *Link {
	c := ch.GetTunnel()
	return &Link{
		Link: models.Link{
			Id:       0,
...
func newPacketLink(ch cool.Tunnel, conn net.PacketConn, addr net.Addr) *Link {
	c := ch.GetTunnel()
	return &Link{
		Link: models.Link{
			Id:       0,
